...
// これはGitHubの使い方やC++学習の復習として考えたプログラムを書き出してみた　、、、みたいなもの。
// 学習用なので今後もアップデートするかもしれないし、別なものに取り組んでいるかもしれない　
// いつかGit_Hubにアップされているこのコードを見て赤面するだろう。
...
int main()
{
    ...
    while (true)
    {
                ...
                if (iter->get_name() == account_name && iter->get_password() == password)
                {
                    ...
                    while (keep_dealing)
                    {
                        ...
                        else if (key == 1)
                        {
                            ...
                            do
                            {
                                std::cout << "引き出し、またはお預入の金額を入力してください。\n";
                                std::cout << "金額:";
                                std::cin >> money;
                                if (std::cin.fail())
                                {
        ...
        else // num = 2でlog_inがfalseの場合、新規アカウントを作成する
        {
            ...
            std::cout << new_account_name << "様のアカウントは作成されました。\n";
